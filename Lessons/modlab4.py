...
def read_date():
    ...
    """
    # Type your code here. #
    input_date = input()  # Read the input string
    year, month, day = map(int, input_date.split('-')) # Split the input string into year, month, and day
    return date(year, month, day)  # Create a date object and return it

# 2. Use read_date() to read four (unique) date objects, putting the date objects in a list
dates = [read_date() for _ in range(4)]
